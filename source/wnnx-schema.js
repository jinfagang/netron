...
$root.wnn.Split = class Split {

    static decode(reader, position) {
        const $ = new $root.wnn.Split();
        $.split_sections = reader.typedArray(position, 4, Int32Array);
        $.dim = reader.int32_(position, 6, 0);
        $.data_format = reader.int8_(position, 8, 0);
        return $;
    }
};
...
$root.wnn.Graph = class Graph {
    ...
    static decode(reader, position) {
        ...
        $.tensor_index_names = reader.strings_(position, 24);
        $.tensor_number = reader.int32_(position, 26, 0);
        $.subgraph = reader.table(position, 28, $root.wnn.SubGraph.decode);
        $.uuid = reader.string_(position, 30, null);
        $.password = reader.string_(position, 32, null);
        return $;
    }
};
